...
export const MFA_STORAGE_KEY_SAVED = "quotio.mfa.vault.v1";
export const MFA_STORAGE_KEY_HISTORY = "quotio.mfa.query.history.v1";

const LEGACY_STORAGE_KEYS_SAVED = ["agtools.mfa.vault.v2", "agtools.mfa.vault.v1", "agtools.two_factor_auth.saved.v2"];
const LEGACY_STORAGE_KEYS_HISTORY = ["agtools.2fa.query.history.v1", "agtools.two_factor_auth.history.v2"];
...
export function toMfaSecretIdentity(secret: string): string {
  return normalizeStrictBase32(secret) || secret.trim().toUpperCase();
}
...
export function parseMfaCredentialInput(rawInput: string): ParsedMfaCredential | null {
  ...
  try {
    ...
    if (parsed instanceof OTPAuth.TOTP) {
      ...
      if (!normalizeStrictBase32(rawSecret)) return null;
      return { accountName: buildAccountDisplayName(parsed.issuer || "", parsed.label || ""), secret: rawSecret };
    }
  } catch {
    // Fall through to raw Base32 parsing.
  }

  if (!normalizeStrictBase32(input)) return null;
  return { accountName: "", secret: input };
}
...
function buildMfaTotp(rawSecret: string): OTPAuth.TOTP | null {
  ...
  if (secretMatch?.[1]) {
    ...
    if (secretPart) {
      const period = Number(raw.match(/(?:^|[?&])period=(\d+)/i)?.[1] || 30);
      const digits = Number(raw.match(/(?:^|[?&])digits=(\d+)/i)?.[1] || 6);
      const algorithm = parseAlgorithm(decodeURIComponent(raw.match(/(?:^|[?&])algorithm=([^&\s]+)/i)?.[1] || ""));
  ...
  return new OTPAuth.TOTP({ secret: OTPAuth.Secret.fromBase32(normalized), period: 30, digits: 6 });
}
...
export function dedupeMfaRecordsBySecret(records: MfaRecord[]): MfaRecord[] {
  ...
  for (const record of sorted) {
    ...
    if (!map.has(identity)) map.set(identity, record);
  }
  return Array.from(map.values());
}

export function loadSavedMfaRecords(): MfaRecord[] {
  const merged = [MFA_STORAGE_KEY_SAVED, ...LEGACY_STORAGE_KEYS_SAVED]
    .flatMap(readStorageArray)
    .map(normalizeMfaRecord)
    .filter((item): item is MfaRecord => Boolean(item));
  return dedupeMfaRecordsBySecret(merged);
}

export function loadMfaHistoryRecords(): MfaRecord[] {
  const merged = [MFA_STORAGE_KEY_HISTORY, ...LEGACY_STORAGE_KEYS_HISTORY]
    .flatMap(readStorageArray)
    .map(normalizeMfaRecord)
    .filter((item): item is MfaRecord => Boolean(item));
